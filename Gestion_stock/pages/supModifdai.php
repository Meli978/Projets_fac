...
include 'fonctions/dai.func.php';
  $id = $_GET['id'];
  foreach(recuperer_produit_dai($id) as $dai){

    $tab_nom = array(unserialize($dai ->nom_produit));
    $tab_qte = array(unserialize($dai ->qte));
    $tab_prix = array(unserialize($dai ->prix));
...
<div class="all">
 ...
 <form  method="post">
  ...
  <header id="header">
    ...
    <span id="date">Date:<input type="date" name="date" value="<?php echo $dai->date ?>"> </span>
  ...
  <section>
    <p>
      <span>Numero de bon: <strong> <?php echo $dai->id ?></strong></span>
    </p>
    ...
    $ligne = count($tab_nom);
      for($i=0; $i<count($tab_nom); $i++){
        ...
        $prix = $tab_prix[$i];
         ...
         ?>
    <tr>
      <td> <input type="checkbox" class="checkClass" name="supp_modif[<?php echo $i ?>]" value=""></td>
      ...
      <td> <input type="text" name="<?php echo "prix_".$i ?>" value="<?php echo $prix; ?>"></td>
  ...
  ///chercher produit qui ont le mm nom
    ...
    //////fin lecture lignes///////////////////
      if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
        ...
        modifier_dai($id,$tab_nom_dai, $tab_qte_dai,$tab_prix_dai,$date);
        ?>
        <script type="text/javascript">
          window.open("index.php?page=supModifdai&id=<?php echo $id; ?>","_self");
        </script>
...
//////fin lecture lignes///////////////////
  if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
    ...
    <script type="text/javascript">
      window.open("index.php?page=AfficherDai&id=<?php echo $id; ?>","_self");
    </script>
