...
$id = $_GET['id'];
  foreach(recuperer_produit_reception($id) as $reception){
...
<div  class = "bouton_search">
  <div id="buttons">
   <button class="btn blue" onclick="ajouter()"><a href="#" id ="ajouter">Ajouter d'autres receptions</a></button>
 </div>
</div>
  ...
  if(isset($_POST["supp_modif"])){
    ...
    //////fin lecture lignes///////////////////
      if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
        ...
        modifier_reception($id,$tab_nom_reception, $tab_qte_reception,$date);
