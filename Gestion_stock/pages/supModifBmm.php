...
$id = $_GET['id'];
  foreach(recuperer_produit_bmm($id) as $bmm){
...
<div class="all">
 <form class="" action="" method="post">
  ...
  <section>
    <p>
      <span>Numero de bon: <strong><?php echo $bmm->id ?></strong></span>
    </p>
  ...
  if(isset($_POST["supp_modif"])){
      ...
      supprimer_bmm($id);
        ?>
         ...
         <script type="text/javascript">
          window.open("index.php?page=bmmDefinitif", "_self");
        </script>
        <?php
    }
    else{
    ...
    //////fin lecture lignes///////////////////
      if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
        ...
        modifier_bmm($id,$tab_nom_bmm, $tab_qte_bmm,$date);
?>
      <script type="text/javascript">
      window.open("index.php?page=bmmDefinitif", "_self");
      </script>
