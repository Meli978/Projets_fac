...
include 'fonctions/bsm.func.php';
  $nb = $_GET['nb_lignes'];
  $nb  = 4;
    foreach(recuperer_nb_reintegration() as $key=>$val){
 ...
 <script type="text/javascript" src="js/reintegration.func.js">
  ...
  for($j=0; $j<$nb; $j++){
    ...
    if(isset($nom_produit) && !empty($nom_produit) && isset($qte_produit) && !empty($qte_produit)){
      ...
      if((produit_existe($nom_produit) == 1) && ($qte_produit > 0)){
          foreach(recuperer_donnes_produit($nom_produit) as $produit){
            $qte_ancienne = ($produit->qte);
          }
          if($qte_ancienne>0 && ($qte_produit<= $qte_ancienne)){
...
//si il na jamais ete inserer, on linsere dans la bdd
        if(!$produit_deja_insere){
           $qte_produit = $qte_produit + $qtePlus;
            ...
            modifier_bdd_reintegration_produit($nom_produit,$qte_produit );
            $tab_nom[] = $nom_produit;
            $tab_qte[] = $qte_produit;
        }
/////////fin qte ancienne//////
...
//////fin lecture lignes///////////////////
  if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
    $tab_nom_reintegration = serialize($tab_nom);
    $tab_qte_reintegration = serialize($tab_qte);
    inserer_reintegration($bon_id,$tab_nom_reintegration, $tab_qte_reintegration,$date);

    ?>

    <script type="text/javascript">
      window.open("index.php?page=supModifReintegration&id=<?php echo $bon_id; ?>", "_self");
    </script>
<?php
  }
  ...
  $chaine = "Veuillez entrer une date";
  echo $chaine;
  ?>
  <script type="text/javascript">
    inserer('<?php echo $chaine; ?>');
