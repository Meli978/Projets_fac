...
include 'fonctions/bmm.func.php';
  $nb = $_GET['nb_lignes'];
  $nb = 4;
    foreach(recuperer_nb_bmm() as $key=>$val){
...
//si il na jamais ete inserer, on linsere dans la bdd
        if(!$produit_deja_insere){
           $qte_produit = $qte_produit + $qtePlus;
            ...
            modifier_bdd_bmm_produit($nom_produit,$qte_produit );
            $tab_nom[] = $nom_produit;
            $tab_qte[] = $qte_produit;
        }
...
//////fin lecture lignes///////////////////
  if(($insertion_impossible == 0) && ($ligne_vide!==$nb) && (!empty($tab_nom)) &&(!empty($tab_qte))){
    $tab_nom_bmm = serialize($tab_nom);
    $tab_qte_bmm = serialize($tab_qte);
    inserer_bmm($bon_id,$tab_nom_bmm, $tab_qte_bmm,$date);

    ?>

    <script type="text/javascript">
      window.open("index.php?page=supModifBmm&id=<?php echo $bon_id; ?>", "_self");
    </script>
<?php
  }
...
?>
<script type="text/javascript" src="js/bmm.func.js"></script>
